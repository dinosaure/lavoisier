...
struct
  ...
  and 'a atom =
    | String    : string atom
    | Bytes     : bytes atom
    | Bigstring : bigstring atom
  and vec = { off : int option
            ; len : int option }
  and 'a blit = 'a -> int -> bigstring -> int -> int -> unit
  and 'a length = 'a -> int
  ...
  let rec encode
    ...
    = fun k e -> function
      ...
      | Blt ({ off; len; }, blit, length) -> (fun v -> Lavoisier.write k ~blit ~length ?off ?len v e )
      ...
      | Flush t -> (fun v -> let k e = Lavoisier.flush k e in encode k e t v)
  and _list
  ...
  let blitter ?off ?len length blit = Blt ({ off; len; }, blit, length)
  ...
  let nop = Nop
  let list ?(sep = nop) a = Lst (sep, a)
  let option t = Opt t
  let compose f g = Seq (f, g)
  let conv fof fto t = Conv (fof, fto, t)

  let const a x = Const (a, x)

  let prefix tp ta = Prefix (tp, ta)
  let suffix ta tp = Suffix (ta, tp)

  let array ?(sep = nop) a = Arr (sep, a)
  let case t fto fof = Case (t, fto, fof)
  let union = function
    | [] -> invalid_arg "union: empty list"
    | cases -> Union cases
  let mu f = Mu f
  let csub t ?off ?len x = Const (Sub ({ off; len; }, t), x)
  let flush t = Flush t
  ...
  struct
    ...
    let string ?off ?len x = csub String ?off ?len x
    let bytes ?off ?len x = csub Bytes ?off ?len x
    let bigstring ?off ?len x = csub Bigstring ?off ?len x
  end
  ...
  let keval : type a r. (encoder -> r state) -> (Lavoisier.IOVec.t list -> int) -> encoder -> a t -> a -> r
    = fun k w e t a ->
      write w (encode k e t a)

  let eval w e t v = keval (fun e -> Lavoisier.End ()) w e (flush t) v

  let keval_lwt
    : type a r. (encoder -> r state) -> (Lavoisier.IOVec.t list -> int Lwt.t) -> encoder -> a t -> a -> r Lwt.t
    = fun k w e t v ->
      let open Lwt.Infix in
...
let rec jsona : json A.t =
  ...
  let json =
    ...
    { run = fun k e -> function
          ...
          | `String s -> (dquote *> string <* dquote).run k e s
          | `A l -> (bracks (list ~sep:comma json)).run k e l
          | `O l -> (braces (list ~sep:comma binding)).run k e l }
  in json
